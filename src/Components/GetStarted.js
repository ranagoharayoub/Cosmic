import React from 'react'
import './GetStarted.scss';

// import SVG from '../images/Group 8187.svg';



function GetStarted() {
    return (
        <div className='getStarted'>
            <div className="rectangle3219"></div>
            <div className="rectangle3219content">A Cutting Edge Automated Business Built With Industry Leading Partners</div>
            <div className="googlecloud"></div>
            <div className="aws"></div>
            <div className="azure"></div>
            <div className="stripe"></div>
            
            <div className='left'>
                <div className='circle'>
                    {/* <img src={Circle} alt="Circle"/> */}
                </div>
                <div className='image'><img src="" alt='pic'/></div>

            </div>
            <div className='mid'>
                <div className='line'>
                    
                </div>
                <div className="icon"></div>

            </div>
            <div className='right'>
                <h3>GET STARTED</h3>
                <h1>Training, Certificate supply chain and operations all in one place</h1>
                <div className="icon2"></div>
                <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Cras ximus vulputate libero nec commoda.</p>
                <div className="button"></div>
                <div className="buttonContent">SIGN UP FREE</div>
                <div className="buttonIcon"></div>
                <div className="group8188"></div>
               
                                     
               
                
                

            </div>
        </div>
    )
}

export default GetStarted
